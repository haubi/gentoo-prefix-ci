...
.\bin\bash.exe --noprofile --norc -c 'exec 2>&1; if [[ ${BUILD_BINARIESDIRECTORY} ]] ; then d=\"$(/bin/cygpath -u \"${BUILD_BINARIESDIRECTORY}\")\" && mkdir -p \"${d}/gentoo-distfiles\" && distdir=\"--gentoo-distdir=${d}/gentoo-distfiles\"; fi; set -x; upload=yes; [[ $(/bin/hostname),$(/bin/uname -r) == vsts19-ssi-01,*-gentoo-* ]] || upload=no; /bin/bash /sources/prefix/staging-bootstrap.sh --sources=/sources --staging=\"$(/bin/cygpath -u \"${BUILD_STAGINGDIRECTORY}\")\" ${BUILD_BINARIESDIRECTORY:+--gentoo-distdir=\"$(/bin/cygpath -u \"${BUILD_BINARIESDIRECTORY}/gentoo-distfiles\")\"} --upload-results=${upload}'
exit $LastExitCode
